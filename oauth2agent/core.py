...
def _unique_strings(obj: Any, keys: set[str]) -> list[str]:
    normalized = {key.lower() for key in keys}
    values: list[str] = []
    seen: set[str] = set()
    for key, value in _walk_values(obj):
        if key.lower() not in normalized or not isinstance(value, str):
            continue
...
def parse_oauth_document(document: Any) -> OAuthMaterial:
    ...
    access_tokens = _unique_strings(document, {"access_token", "accessToken"})
    if not access_tokens:
        raise OAuth2AgentError("OAuth JSON does not contain access_token")
    if len(access_tokens) > 1:
        raise OAuth2AgentError(
            "OAuth JSON contains multiple distinct access_token values; export or select exactly one account before converting"
        )
    access_token = access_tokens[0]

    id_tokens = _unique_strings(document, {"id_token", "idToken"})
    if len(id_tokens) > 1:
        raise OAuth2AgentError(
            "OAuth JSON contains multiple distinct id_token values; export or select exactly one account before converting"
        )
    id_token = id_tokens[0] if id_tokens else None
...
def _json_request(
    ...
    *,
    ...
    if json_body is not None and form_body is not None:
        raise OAuth2AgentError("request cannot contain both json_body and form_body")
